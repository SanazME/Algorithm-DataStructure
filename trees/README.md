...
- We can use DP or BFS to solve it. For BFS, we start from zero and keep adding all coins and at each level of the tree check if we reach the amount and if so we return the number of coins that we used.
```py
